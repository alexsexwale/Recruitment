...
router.post("/activate", verifyToken, (req, res, next) => {
  if (req.body.TransactionAccepted && req.body.Extra1 && req.body.Extra2) {
    ...
    res
      .status(200)
      .redirect(
        "https://joboxstaging.web.app/client/payment/success/" + req.body.Extra1
      );
  } else {
...
// Cancel payment
router.post("/cancelPayment", verifyToken, (req, res, next) => {
  if (req.body.Extra1) {
    res
      .status(200)
      .redirect(
        "https://joboxstaging.web.app/client/jobs/micro/status/" +
          req.body.Extra1
      );
  } else {
    db.collection("errors").add({
...
// Decline payment
router.post("/decline", verifyToken, async (req, res, next) => {
  if (req.body.Extra1) {
    ...
    var msg = {
      ...
      text:
        " The user with the job id: " +
        req.body.Extra1 +
        ", was unable to process the payment. The payment has been declined."
    };
    sgMail.send(msg);
    res
      .status(200)
      .redirect(
        "https://joboxstaging.web.app/client/payment/fail/" + req.body.Extra1
      );
  } else {
    db.collection("errors").add({
      created: moment(Date.now()).format("L"),
